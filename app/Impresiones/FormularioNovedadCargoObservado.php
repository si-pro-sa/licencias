...
class FormularioNovedadCargoObservado extends PDF
{
    private $data;
    ...
    public function Header()
    {
        ...
        $this->setY(10);
        $this->createCeldaConTexto(0,340,1,'DIRECCIÓN GENERAL DE RRHH EN SALUD',false);
        $this->newRow();
        $this->createCeldaConTexto(0,340,1,'DIRECCIÓN DE PLANIFICACIÓN Y DESARROLLO',false);
        $this->newRow();
        $this->createCeldaConTexto(0,340,2,'DEPARTAMENTO DE PLANIFICACIÓN',false);
        $this->newRow();
        $this->createCeldaConTexto(0,340,1,'EFECTOR: '.$this->data['efector'].' - CARGA LOCAL DE DATOS',false);
        $this->newRow();
        $this->createCeldaConTexto(0,340,1,'PLANILLA DE OBSERVACIONES - REEMPLAZOS',false);
        $this->newRow();
        $this->createCeldaConTexto(0,340,1,'MES DE LIQUIDACIÓN: '.$this->data['periodo_liq'],false);
        $this->drawLogo();
        $this->newRow();
        ...
        $this->createCeldaConTexto(115,150,2,'APELLIDO Y NOMBRE DEL REEMPLAZANTE',true);
        ...
        $this->createCeldaConTexto(230,290,2,'TIPO DE REEMPLAZO',true);
        $this->createCeldaConTexto(290,330,2,'OBSERVACIONES',true);
    ...
    public function Table() {
        ...
        foreach ($this->formularios as $key => $formulario) {
            ...
            $this->createCeldaConTexto(290,330,1.3,$formulario['observaciones']);
    ...
    public static function imprimir($data,array $formularios)
    {
        ...
        $pdf->Output('D', 'novedades_observadas_LIQ_'.$data['efector'].'_'.time().'.pdf');
    }
