...
class FormularioPsicotecnicoPDF extends PDF
{
    ...
    public function Header()
    {
        if ($this->idtipo_entrevista === 1){
            $registro_txt = utf8_decode('REGISTRO RESUMEN DE EVALUACIÓN');
        }else{
        ...
        $x = $this->GetX();
        $y = $this->GetY();
        $this->MultiCell(72,$altura_recuadro/2,utf8_decode('REGISTRO RESUMEN DE EVALUACIÓN'),1, 'C');
        $this->SetXY($x+72,$y);
        $this->SetFont('Arial', '', 9);
        $this->MultiCell(65, $altura_recuadro/5, utf8_decode('Código: F-5.3
Fecha: 01/02/2020
Revisión: 03
...
Página ' . $this->PageNo()), 1, 'L');
    ...
    public function DatosPostulante($postulante) : void
    {
        $this->SetFont('Arial', 'BI', 10);
        $this->SetFillColor(79,129,189);
        $this->SetTextColor(255,255,255);
        $this->Cell(190, 5, 'Datos del postulante', 1, 1, 'C',true);
        ...
        $this->Cell(95,4,utf8_decode('SEXO: '.$postulante['sexo']),0,0, 'L');
        $this->ln(7);
        $this->Cell(95,4,utf8_decode('FORMACIÓN: '.$postulante['formacion']),0,0, 'L');
        $this->Cell(95,4,utf8_decode('POSTULA PARA: '.$postulante['puesto_postulado']),0,0, 'L');
        $this->ln(7);
    ...
    public function PerfilLaboral($perfil_laboral): void
    {
        $y = 117;
        $alto = 8;
        $this->SetY($y);
        ...
        $this->SetFillColor(79,129,189);
        $this->SetTextColor(255,255,255);
        $this->Cell(190, 5, utf8_decode('Evaluación de perfil laboral'), 1, 1, 'C',true);
        $this->SetTextColor(0,0,0);
        $this->Cell(190,73, '',1,1);
        $this->Line(105,$y+5,105,$y+78);

        $y += 5;
        $this->SetY($y);
        $this->SetFont('Arial', '', 9);
        $this->Cell(50,$alto,utf8_decode('Aspectos cognitivos'),1,1, 'L');
        $this->Cell(50,$alto,utf8_decode('Aspectos psicoafectivos'),1,1, 'L');
        $this->Cell(50,$alto,utf8_decode('Desempeño en evaluación'),1,1, 'L');
        $this->Cell(50,$alto,utf8_decode('Motivación inferida'),1,2, 'L');

        $this->SetXY($this->GetX()+50,$y);
        $this->dibujarEscalas($perfil_laboral,$alto,$this->escalaPerfilLaboral);

        $this->SetFont('Arial', '', 9);
        $lineas = $this->contarLineas(70,16,utf8_decode('¿Refiere experiencia laboral vinculada al puesto? '));
        $x = $this->GetX();
        $this->SetXY($x+95,$y);
        $this->MultiCell(70,16/$lineas,utf8_decode('¿Refiere experiencia laboral vinculada al puesto? '),1,2);
        $this->SetFillColor(79,129,189);
        $this->SetXY($x+165,$y);
        $this->Cell(12.5,16, 'SI',1,2, 'C', $perfil_laboral['experiencia_laboral']);
        $this->SetXY($x+177.5,$y);
        $this->Cell(12.5,16, 'NO',1,2, 'C', !$perfil_laboral['experiencia_laboral']);

        $y += 16;
        $this->SetXY($x+95,$y);
        $this->MultiCell(70,16,utf8_decode('¿Tiene experiencia en el sector público? '),1,2);
        $this->SetFillColor(79,129,189);
        $this->SetXY($x+165,$y);
        $this->Cell(12.5,16, 'SI',1,2, 'C',$perfil_laboral['sector_publico']);
        $this->SetXY($x+177.5,$y);
        $this->Cell(12.5,16, 'NO',1,2, 'C',!$perfil_laboral['sector_publico']);

        $y += 16;
        $this->setXY($this->GetX()+95,$y);
        $this->Cell(95,41, '',1,1, 'L');
        $this->setXY($this->GetX()+95,$y);
        $this->Cell(95,6, 'Observaciones:',0,1, 'L');
        $this->SetFont('Arial', 'B', 9);
        $y += 6;
        $this->SetXY($this->GetX()+95,$y);
        $this->SetFont('Arial', '', 9);
        $this->MultiCell(85,4,utf8_decode(preg_replace("/[\r\n|\n|\r]+/", ' ', $perfil_laboral['observaciones'])));
    }
    ...
    {
        $this->SetXY(155,252);
        $this->Image(public_path('images/firmas/' .$firma['firma']),140,240,63,36);
        $this->SetFont('Arial','B',12);
        $this->SetXY(140,267);
