...
class Agente extends Model
{
    ...
    {
        return $this->belongsTo('App\Models\Domicilio', 'iddomicilio')->withDefault([
            'iddomicilio' => null,
            'calle' => '',
            'numero' => '',
            'piso' => '',
    ...
    {
        $cantidadCargos = DB::table('cargos as c')
            ->join('puestos as p', 'c.idpuesto', '=', 'p.idpuesto')
            ->join('agente as a', 'p.idagente', '=', 'a.idagente')
            ->join('cargo_cambio_estados as cce', 'c.idcargo', '=', 'cce.idcargo')
            ->where('a.idagente', $this->idagente)
            ->where('cce.idcargo_tipo_formulario', '<', '3')
            ->where('cce.idcargo_tipo_visado', '=', '8')
            ->count();

        return $cantidadCargos > 0;
    }
